...
#' optionally also computes the confidence intervals around those estimates, using a
#' normal approximation \eqn{mean ± Z_{\alpha}\times SE} for continuous values
#' and `stats::binom.test()` exact binomial intervals for discrete (i.e. character,
#' factor, logical) ones.
#'
#' @details The function is passed as the `est_fun` argument of
#'   `tidy_tree()`, but works also as stand-alone. This is a default estimation
...
#' @param interval_level Confidence level. Must be strictly greater than 0 and less
...
#' ## (actually esl_fun = get_pred_estimates is redundant since it's the default)
#'
#'
#'
